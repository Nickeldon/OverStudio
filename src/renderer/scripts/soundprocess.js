...
if (!localStorage.getItem("vol")) {
  ...
  if (Math.max(vol) === 0) {
    document.getElementById("vol-btn").src = "../../ressources/icons/SVG/mute.svg";
  } else if (Math.max(vol) === 20) {
    document.getElementById("vol-btn").src = "../../ressources/icons/SVG/max.svg";
  } else {
    document.getElementById("vol-btn").src = "../../ressources/icons/SVG/volume.svg";
  }
}
...
document.getElementById("shuffle").addEventListener("click", () => {
  ...
  if (PlayBackMode === "linear") {
    ...
    document.getElementById("shuffle").src = "../../ressources/icons/SVG/shuffle.svg";
    ...
    document.getElementById("shuffle").src =
      "../../ressources/icons/SVG/shuffleSmart.svg";
  } else if (PlayBackMode === "shuffle" && ShuffleSmartEnabled) {
    PlayBackMode = "loop";
    ShuffleSmartEnabled = false;
    document.getElementById("shuffle").style.transition = "transform 0s";
    document.getElementById("shuffle").style.transform = "rotate(0deg)";
    document.getElementById("shuffle").src = "../../ressources/icons/SVG/loop.svg";
  } else {
    PlayBackMode = "linear";
    document.getElementById("shuffle").style.transition = "transform 0s";
    document.getElementById("shuffle").style.transform = "rotate(-90deg)";
    document.getElementById("shuffle").src = "../../ressources/icons/SVG/linear.svg";
  }
});
...
class foundSearchObject{
  constructor({title, artist}){
    this.title = {
      temp: title?.toLowerCase(),
      final: title};
    this.artist = {
      temp: artist?.toLowerCase(),
      final: artist};
  }
}
  ...
  if (input && playlist) {
    if (playlist.length > 0) {
      ...
      playlist.forEach((track) => {
        ...
        let result = new foundSearchObject({
          title: track.title,
          artist: track.artist
        })
        let PLId = playlist.indexOf(track);

        let highlightcolor = `color: ${themes[theme].highlight.search}`
        if (result.title.temp && result.artist.temp) {
          console.log(result.title.temp, result.artist.temp)
          if ((result.title.temp).includes(input) || (result.artist.temp).includes(input)) {
            let highlightedTitle = result.title.final.replace(new RegExp(input, 'gi'), match => `<span style="${highlightcolor}">${match}</span>`);
            let highlightedArtist = result.artist.final.replace(new RegExp(input, 'gi'), match => `<span style="${highlightcolor}">${match}</span>`);
            searchResults.push({
              title: highlightedTitle,
              artist: highlightedArtist,
              id: PLId,
            });
          }
        } else if (result.title.temp) {
          if ((result.title.temp).includes(input)) {
            let highlightedTitle = result.title.final.replace(new RegExp(input, 'gi'), match => `<span style="${highlightcolor}">${match}</span>`);
